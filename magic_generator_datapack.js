artificer.adjective = "Grimy, Chipped, Warm, Rough, Smooth, Old, Ancient, Crumbling, Pristine, Cool, Ornate, Plain, Chilling, Runed, Carved, Decorated, Delicate, Burned, Shining, Smokey".split(", ");
artificer.origin = "draconic, dwarven, elven, pre-humanoid, divine, unholy, abyssal, otherworldly, royal, orcish, undead, goblinoid, dark elven, astral, ethereal, hellish, elemental, gnomish, shadow, arcane".split(", ");
artificer.item = "rod, gemstone, opal, stone, coin, crown, amulet, ring, dagger, brooch, glove, mirror, hammer, pipe, tooth, flute, mask, bell, lamp, bone".split(", ");

artificer.power = [
"offers brief glimpses of the future",
"provides true seeing",
"engulfs the user in protective flames",
"allows passage through solid objects",
"provides limited telekenesis",
"lets the user emit ghost sounds",
"lets the user communicate telepathically",
"lets the user draws someone closer",
"lets the user push someone away",
"turns user into unbreakable stone",
"turns the user's skin into flexible steel",
"lets the user levitate",
"engulfs a weapon in flame",
"creates an antimagic shell",
"speeds up the user",
"increases the user's luck",
"lets the user talk to plants",
"provides historical visions of the current location",
"lets the user see through the eyes of another",
"lets the user create and control an illusional doppelganger",
"creates acidic fog",
"lets the user walk on air",
"creates a magical alarm",
"lets the user alter their appearance",
"creates wild growth",
"enlarges the user",
"shrinks the user",
"lets the user open a dimensional door",
"makes the user invisible",
"lets the user soothe someone",
"lets the user enrages someone",
"lets the user create an arcane lock",
"projects the user into the astral plane",
"lets the user summon tentacles",
"creates food and drink",
"lets the user call lightning",
"lets the user create fear",
"lets the user charm someone",
"lets the user summon a large magical fist",
"lets the user command plants",
"answers a user's question",
"lets the user emit a cone of cold",
"lets the user cure diseases and curses",
"lets the user create darkness",
"lets the user anchor teleportation",
"lets the user dispel magic",
"lets the user summon swarms",
"lets the user drain energy",
"lets the user create an explosive rune",
"lets the user finds traps",
"lets the user fires flaming arrows",
"gives the user gaseous form",
"protects the user from spells",
"lets the user create an illusion",
"lets the user heat metal",
"lets the user create an icy storm",
"helps the user solves puzzles",
"lets the user summon a faithful servant",
"helps the user opens locks",
"lets the user locate an object",
"lets the user locate a creature",
"lets the user create a shell of magical protection",
"lets the user create a safe place",
"lets the user fire magical missiles",
"lets the user break inanimate objects",
"lets the user summon a spectral steed",
"lets the user create an obscuring mist",
"lets the user summon nightmares",
"lets the user polymorph an object",
"lets the user create a wall of rock",
"lets the user read any language",
"lets the user reverse gravity",
"lets the user fire a scorching ray",
"lets the user create a blinding light",
"lets the user walk in shadows",
"lets the user silence an area",
"lets the user slow a creature",
"gives the user spider climbing",
"lets the user summon a spiritual weapon",
"lets the user impart a suggestion",
"lets the user stop time",
"lets the user strike true",
"lets the user create a wall of fire",
"lets the user create an invisible wall",
"lets the user allows water breathing",
"lets the user walk on water",
"lets the user summon webs",
"lets the user control a rope",
"lets the user secure a door",
"lets the user make someone sleepy",
"makes the user fall like a feather",
"lets the user detect motives",
"increases the user's charisma",
"increases the user's strength",
"increases the user's dexterity",
"increases the user's intelligence",
"increases the user's wisdom",
"increases the user's constitution",
"shines like the sun",
"lets the user turn rock to mud"
];

artificer.quirk = [
"forces the user to speak the truth",
"is uncomfortably hot",
"emits bright harmless flames",
"whispers horrible truths",
"turns the user's skin blue",
"glows in bright white light",
"the user sees spiders crawling on all living things",
"the user sees silent angry ghosts",
"shouts angrily at children",
"makes the user clumsy",
"makes the user itchy",
"vexes the user",
"drips blood",
"shocks the user randomly",
"gives the user a headache",
"smells horrible",
"makes the user nauseous",
"smokes",
"drips with oil",
"shines with multicolored light",
"the user giggles uncontrollably",
"deafens the user",
"increases the user's sensitivity",
"covers the user in purple",
"turns the user's skin red",
"turns the user's skin green",
"makes the user's skin transparent",
"the user is unable to do math",
"the user can only sing when speaking",
"the user's hair stands on end",
"gives user hiccups",
"garbles the user's speech",
"sees everything in xray",
"user experiences vertigo",
"user's eyes go totally black",
"user's eyes glow white",
"user begins to weep",
"the user continually tastes blood",
"the user grows a thick beard",
"renders the user mute",
"emits the stench of decaying meat",
"insults those around in the user's voice",
"causes the user to drool",
"causes a ringing in the user's ear",
"user's flesh becomes magnetic",
"smells like freshly baked bread",
"makes the user dizzy",
"the user grows excessive body hair",
"is uncomfortably cold to the touch",
"unbalances the user",
"is slimy to the touch",
"attracts mindless monsters",
"user becomes depressed",
"sets one of the user's possessions ablaze",
"fills the user with euphoria",
"gives the user an uncontrollable muscular tick",
"summons uncontrollable rats",
"attracts insects",
"the user appears 50 years older",
"the user's bladder always feels full",
"the user becomes slightly drunk",
"user gains acute hearing",
"shakes vigorously",
"the user is dehydrated",
"the user gets chills",
"the user sweats heavily",
"the user sneezes sporadically",
"the user gets the sniffles",
"the user becomes paranoid",
"the user becomes tired",
"the user becomes lethargic",
"the user's eyes move independently",
"the user becomes colorblind",
"the user becomes a child",
"the user cannot read",
"the user cannot stop talking",
"the user becomes hungry",
"the user can only speak the language of the item's origin",
"the user loses its temper",
"the user gains weight",
"the user grows a third eye",
"the user grows a tail",
"the user becomes nearsighted",
"the user can only speak in a whisper",
"the user grows whiskers",
"the user grows a tail",
"the user grows tusks",
"the user's skin becomes scales",
"the user moves slowly",
"the user is overly optimistic",
"the user becomes self conscious",
"the user believes they're a god",
"the user becomes claustrophobic",
"the user loses a tooth",
"the user's extremities go numb",
"the user appears undead",
"grass grows on the user's skin",
"the user grows claws",
"the user gains temporary amnesia",
"the user becomes afraid of heights"
];
